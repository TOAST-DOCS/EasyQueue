...
ユーザーは Kafka トピックを通じてアプリケーション間のデータを非同期的に発行およびサブスクライブし、柔軟なデータパイプラインを簡単に構成できます。
また、メッセージはクラスター内に分散保存および多重レプリケーションされるため、障害が発生した場合でもデータ損失を防ぎ、受信アプリケーションが一時停止した場合でも、キュー (queue) に保管されたメッセージを通じて安定した処理を保証します。
